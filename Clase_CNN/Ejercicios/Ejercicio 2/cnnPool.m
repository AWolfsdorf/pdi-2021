function pooledFeatures = cnnPool(poolDim, convolvedFeatures)
    ...
    for imageNum = 1:numImages
        for featureNum = 1:numFilters
            pooledFeatures(:, :, featureNum, imageNum) = downsampling(convolvedFeatures(:, :, featureNum, imageNum), poolDim);
        end
    end
end

function smallImage = downsampling(bigImage, filterSize)
    averagingFunction = @(x) mean(x.data(:));
    smallImage = blockproc(bigImage, [filterSize, filterSize] , averagingFunction);
end